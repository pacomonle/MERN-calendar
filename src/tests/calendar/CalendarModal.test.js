...
describe('Pruebas en <CalendarModal />', () => {
    ...
    test('debe de mostrar error si falta el titulo', () => {
     // venimos de un closeModal por lo que la caja d etexto del input esta vacia
        wrapper.find('form').simulate('submit', {
            preventDefault(){}
        });
...
// al crear un nuevo evento el activeEvent debe de ser null
        const initState = {
            calendar: {
                events: [],
                activeEvent: null
    ...
    // simular evento -> disparar modificacion del input
  ...
  // venimos de un closeModal por lo que la caja d etexto del input esta vacia
  // hay que disparar de nuevo el input
        wrapper.find('input[name="title"]').simulate('change',{
            target: {
                name: 'title',
                value: 'Hola pruebas'
